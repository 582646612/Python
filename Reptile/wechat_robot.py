...
class WxBot:
    ...
    def _get_uid(self):
        ...
        if uid_info.status_code == 200:
            uid_detail = re.match(r'window.QRLogin.code = (.*); window.QRLogin.uuid = "(.*)";', uid_info.text,
                                  re.M | re.I)
    ...
    def _sync_check(self):
        ...
        url = pre_url + '/synccheck?r=%i&skey=%s&sid=%s&uin=%i&deviceid=%s&synckey=%s&_=%i' % \
              (self._get_rand_num(), self.s_key, self.wei_xin_sid, self.wei_xin_uin, self.device_id, self.sync_key_str,
               now)
        try:
