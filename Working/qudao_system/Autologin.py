...
from Working import run_rengong,run
...
def menu_city(city):
    if city=='怒江' or city=='保山' or city=='丽江' or city=='昭通' or city=='玉溪' or city=='德宏' or city=='文山' or city=='迪庆':
        return '5'
    elif city=='曲靖' or city=='普洱' or city=='楚雄' or city=='红河' or city =='昆明':
        return '4'
...
def boollogin(driver):
    try:
        ...
        return 0
#登录
def get_cookie(name,pwd,menu,local,type):
    print(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()), end=' ')
    print("开始运行,正在打开浏览器……")
    ...
    while login_bool == 1:
        ...
        while len(yzm) != 4:
            ...
            cropped = img[450:471, 975:1050]  # 裁剪坐标为[y0:y1（上下）, x0:x1（左右）]
    ...
    if type=='run':
        run(local,cook)
    elif type=='run_rengong':
        run_rengong(local,cook)
    print(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()), end=' ')
    print("正在退出浏览器……")
    driver.quit()

def auto_login(local,tity,type):
    info = get_namepwd()
    cities = trans_city(tity)
    menu = menu_city(tity)
    name, password = info[cities[0]], info[cities[1]]
    get_cookie(name, password, menu,local,type)
